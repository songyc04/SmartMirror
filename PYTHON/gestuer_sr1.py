...
# ── 제스처 쿨다운 및 타이머 설정 ──
SWIPE_COOLDOWN = 0.8
VOLUME_COOLDOWN = 0.35
HOLD_REQUIRED_TIME = 1.5
DIRECTION_TRIGGER_DISTANCE = 0.10
DIRECTION_DOMINANCE_RATIO = 1.25
DIRECTION_MAX_TIME = 1.0
DIRECTION_REARM_DISTANCE = 0.035
...
def get_palm_center(landmarks):
    ...
    x = sum(landmarks[i].x for i in palm_points) / len(palm_points)
    y = sum(landmarks[i].y for i in palm_points) / len(palm_points)
    return x, y
    ...
    while gesture_running:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # ── 볼륨 및 패널 스와이프 제어 로직 ──
                # 손목 한 점보다 손바닥 중심이 흔들림에 강해 방향 제스처 오인식이 줄어듭니다.
                current_x, current_y = get_palm_center(landmarks)

                if prev_x != 0 and prev_y != 0:
                    if gesture_start_x is None or gesture_start_y is None:
                        gesture_start_x = current_x
                        gesture_start_y = current_y
                        gesture_start_time = now

                    total_x_diff = current_x - gesture_start_x
                    total_y_diff = current_y - gesture_start_y
                    abs_x = abs(total_x_diff)
                    abs_y = abs(total_y_diff)

                    if now - gesture_start_time > DIRECTION_MAX_TIME:
                        gesture_start_x = current_x
                        gesture_start_y = current_y
                        gesture_start_time = now
                        swipe_locked = False
                    elif swipe_locked:
                        if abs_x < DIRECTION_REARM_DISTANCE and abs_y < DIRECTION_REARM_DISTANCE:
                            swipe_locked = False
                            gesture_start_x = current_x
                            gesture_start_y = current_y
                            gesture_start_time = now
                    elif now - last_swipe_time > SWIPE_COOLDOWN:
                        if abs_x >= DIRECTION_TRIGGER_DISTANCE and abs_x >= abs_y * DIRECTION_DOMINANCE_RATIO:
                            if total_x_diff < 0:
                            ...
                            gesture_start_y = current_y
                            gesture_start_time = now
                        elif abs_y >= DIRECTION_TRIGGER_DISTANCE and abs_y >= abs_x * DIRECTION_DOMINANCE_RATIO:
