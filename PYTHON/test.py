...
# ── 공유 데이터 공간 (시각화 연동용 추가) ───────────────────────────────────
# 기존 로직을 건드리지 않고, 메인 GUI 스레드에서 인식 결과를 그려주기 위한 락과 변수입니다.
drawing_lock = threading.Lock()
latest_hands_results = None
latest_face_results = None
...
def voice_controller_thread():
    """ 실시간으로 마이크를 감시하며 '거울아'와 음악 검색 명령을 처리하는 스레드 """
...
def face_position_thread():
    global face_position_running, last_user_position, latest_face_results
    ...
    while face_position_running:
        ...
        # 기존 로직을 유지하면서 팝업창 렌더링용으로 객체를 안전하게 공유합니다.
        with drawing_lock:
            latest_face_results = results
...
def gesture_processing_thread(hands):
    global latest_rgb_frame, gesture_running, qt_connected, latest_hands_results
...
def main():
    global latest_rgb_frame, gesture_running, face_position_running
    ...
    # 시각화(그리기) 전용 MediaPipe 유틸리티 설정 초기화
    mp_drawing = mp.solutions.drawing_utils
    mp_drawing_styles = mp.solutions.drawing_styles

    print(f"\n🚀 [제스처/음성/위치 통합 AI 엔진] 가동 완료. 모니터링 및 팝업 활성화...")
    ...
    while cap.isOpened():
        ...
        # ── 📺 [카메라 시각화 팝업 구현부] ──────────────────────────────────
        # 백그라운드 스레드에 전혀 방해를 주지 않도록 원본 프레임을 복사(clone)하여 그립니다.
        display_frame = frame.copy()
        h, w, _ = display_frame.shape

        with drawing_lock:
            # 1. 손 랜드마크 팝업창에 그리기
            if latest_hands_results and latest_hands_results.multi_hand_landmarks:
                for hand_landmarks in latest_hands_results.multi_hand_landmarks:
            ...
            # 2. 얼굴 바운딩 박스 팝업창에 그리기
            if latest_face_results and latest_face_results.detections:
                for detection in latest_face_results.detections:
                    bbox = detection.location_data.relative_bounding_box
                    xmin = int(bbox.xmin * w)
                    ymin = int(bbox.ymin * h)
                    width = int(bbox.width * w)
                    height = int(bbox.height * h)
                    # 얼굴 영역에 녹색 사각 가이드라인 드로잉
                    cv2.rectangle(display_frame, (xmin, ymin), (xmin + width, ymin + height), (0, 255, 0), 2)
                    
                    # 좌우 분할 기준선 및 상태 텍스트 출력
                    face_center_x = bbox.xmin + bbox.width / 2.0
                    pos_text = "LEFT Zone" if face_center_x < 0.5 else "RIGHT Zone"
                    cv2.putText(display_frame, pos_text, (xmin, ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

        # 팝업 윈도우 생성 및 프레임 노출
        cv2.imshow("AI Mirror Monitor (Face & Gesture)", display_frame)
        # ──────────────────────────────────────────────────────────────────
    ...
    if qt_socket:
        qt_socket.close()
    cap.release()
    cv2.destroyAllWindows() # 모든 OpenCV 팝업 윈도우 정리
